...
## Current Checkpoint / 当前状态

当前实现已完成 Stage 20：自定义 Prompt 模板增强。开源准备阶段将仓库文档、许可证、贡献指南、安全策略、GitHub 模板、`.gitignore` 和发布前检查补齐，但不改变 App 业务行为。

Stage 21 仍然是未来扩展方向集合，不代表已经决定接入 AI API、RAG、macOS 或云同步。进入 Stage 21 前，应先拆成更小的设计提案，并继续保持 local-first 和隐私边界。
...
补上真实使用中最容易卡住的两个入口：错题能保存题图，Today 计划能生成正式任务。
...
## Stage 20.1：Open Source Readiness / 开源准备

当前进度：Stage 20.1 已完成文档与仓库元信息准备。

### 目标

把项目从个人开发状态整理为可以公开阅读、构建、评估和贡献的开源仓库，同时不改变 App 业务行为。

### 已完成内容

- README 改为开源项目首页，明确项目定位、当前状态、功能范围、技术栈、启动方式、隐私边界和文档入口。
- 新增文档索引与架构文档，帮助新贡献者理解目录、数据流、备份边界和 Prompt 系统。
- 更新产品说明、数据模型、UX 流程、Prompt 模板、备份格式、恢复策略、恢复测试和 QA checklist，使其适合公开仓库阅读。
- 新增 MIT `LICENSE`、`CONTRIBUTING.md`、`SECURITY.md`、`CODE_OF_CONDUCT.md` 和 `CHANGELOG.md`。
- 新增 GitHub issue / pull request 模板。
- 扩展 `.gitignore`，覆盖 Xcode 构建产物、用户状态、签名材料、本地数据库、日志、导出备份和密钥文件。
- 保留 fixture 数据为合成样本，不纳入真实学习数据或个人备份。

### 验收标准

- 新用户能从 README 找到构建方式、功能范围和文档入口。
- 贡献者能从 CONTRIBUTING / ARCHITECTURE / QA checklist 理解如何提交小改动。
- `.gitignore` 不会意外放行私有数据、真实备份、签名材料或本地数据库。
- 敏感信息扫描和 `git add -n .` 不显示真实密钥或私有数据。
- Debug 构建通过。
