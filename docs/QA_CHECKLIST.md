...
- [ ] 搜索正常（按 title / description / category）。
- [ ] 搜索大小写不敏感，中文正常匹配。
- [ ] 搜索 + 分类筛选组合正常。
- [ ] 空搜索时显示当前分类全部模板。
- [ ] 无搜索结果时显示”没有找到匹配的 Prompt 模板。”。
- [ ] 旧模板能 upsert 更新（内容更新但 usageCount 保留）。
- [ ] usageCount 不丢失。
- [ ] 用户自定义模板不被覆盖。
- [ ] 变量填写正常。
- [ ] 变量为空时使用”未提供”。
...
- [ ] 常用 Prompt 区空状态提示”复制几次模板后，这里会出现常用 Prompt。”。
- [ ] 最近使用点击能打开模板详情。
- [ ] 最近使用模板被删除后点击提示”这个模板当前不存在，可能已被移除或重命名。”。
- [ ] 搜索 + 分类筛选时不显示常用/最近使用区域。
- [ ] 从错题生成 Prompt 能预填变量。
- [ ] 错题 Prompt 入口仍能找到”错题手术”模板。
- [ ] 每日复盘 Prompt 入口仍能找到”每日复盘”模板。
- [ ] 周复盘 Prompt 入口仍能找到”周复盘”模板。
- [ ] 只有题图无题面时提示正确。
- [ ] 无模板时提示检查 seed。
- [ ] 可以新建自定义模板。
- [ ] 自定义模板能搜索/筛选。
- [ ] 自定义模板能生成 Prompt。
- [ ] 自定义模板复制后 usageCount 增加。
- [ ] 内置模板不能直接编辑。
- [ ] 内置模板能复制为自定义模板。
- [ ] 自定义模板能编辑。
- [ ] 自定义模板能删除且有确认。
