...
从 Stage 20 开始，用户可以创建、编辑和删除自己的 Prompt 模板。

### 功能

- **新建自定义模板**：用户可以从头创建自己的 Prompt 模板。
- **从内置模板复制**：用户可以从内置模板复制一份，修改后保存为自定义模板。
- **编辑自定义模板**：用户可以编辑自己创建的模板，包括标题、分类、说明、变量和正文。
- **删除自定义模板**：用户可以删除自己创建的模板，删除前会有确认提示。
- **变量提取辅助**：编辑器可以从模板正文中自动提取 `{{variableName}}` 格式的变量。

### 保护机制

- **内置模板不可直接编辑**：内置模板（`isBuiltIn == true`）不能直接编辑或删除。
- **内置模板可复制**：用户可以从内置模板复制一份，修改后保存为自定义模板。
- **自定义模板不会被覆盖**：内置模板 upsert 更新时，只更新 `isBuiltIn == true` 的模板，用户自定义模板（`isBuiltIn == false`）永远不会被覆盖或删除。

### 变量格式

- 变量使用 `{{variableName}}` 格式。
- 变量列表支持逗号或换行分隔。
- 编辑器可以从模板正文自动提取变量，并填入变量列表。

### 备份与恢复

- 自定义模板会随 `PromptTemplate` 一起导出到本地备份 JSON。
- 内置模板和自定义模板都会进入备份。
- 恢复时，自定义模板按 `merge-with-new-ids` 策略处理，避免 ID 冲突。
