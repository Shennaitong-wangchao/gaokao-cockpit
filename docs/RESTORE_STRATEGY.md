...
Stage 14 的目标是为未来真正导入恢复做准备；Stage 15 在此基础上修正 restore plan 的风险分类语义，尤其是 invalid reference 的表达方式。当前阶段仍然只做策略、fixture、预检与 restore plan。本阶段不做真实恢复，不写入 SwiftData，不恢复错题图片文件，也不覆盖任何用户数据。

## 当前阶段声明

- Stage 15 不做真实恢复。
- Stage 15 只做恢复策略文档、fixture、导入 dry-run 和 restore plan 预览的语义修正。
- Stage 15 不调用 `ModelContext.insert`、`delete` 或 `save`。
- Stage 15 不把图片写入 `Application Support/MistakeImages/`。
- Stage 15 不接 AI API、不做云同步、不做账号、不做加密、不做 zip。
...
Stage 15 起，invalid reference 不再默认等同于 skipped。它在 restore plan 中被单独归入 `referenceRepairSummary`（needsReview / referenceRepairNeeded），以便未来真实恢复时可以选择：
...
Stage 15 不执行以上文件写入，只统计 `incomingImages`、`validImages`、`missingImages` 和 `estimatedBytes`。
