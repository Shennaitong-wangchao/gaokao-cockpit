...
当前已完成 Stage 4B：FocusSession 专注计时 MVP。

当前版本保留 local-first SwiftData 存储，Today 页面可以获取或创建今日 DayPlan、编辑今日计划草稿、查看今日 StudyTask 摘要与列表，并快速新增今日任务；Tasks 页面已成为正式任务管理入口，支持新增、编辑、删除、状态切换、实际用时和产出备注，并可以从任务进入专注计时。专注结束后会生成 FocusSession，并同步 StudyTask 的实际用时、状态和产出备注。Stage 2 Debug 入口仍保留为折叠区块，后续阶段会移除。
