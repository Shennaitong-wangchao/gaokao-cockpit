...
当前已完成 Stage 7：Daily Review / Weekly Review 复盘 MVP。

当前版本保留 local-first SwiftData 存储，Today 页面可以获取或创建今日 DayPlan、编辑今日计划草稿、查看今日 StudyTask 摘要与列表，并快速新增今日任务；Tasks 页面已成为正式任务管理入口，支持新增、编辑、删除、状态切换、实际用时和产出备注，并可以从任务进入专注计时。专注结束后会生成 FocusSession，并同步 StudyTask 的实际用时、状态和产出备注。Mistakes 页面已成为正式错题手术入口，支持记录、编辑、筛选、删除错题并维护复习状态，也可以从错题生成“错题手术”Prompt。Prompt 页面已成为正式 Prompt 仓库入口，支持按分类筛选内置模板、填写变量、生成完整 Prompt 并复制到剪贴板。Reviews 页面已成为正式复盘入口，支持每日复盘、周复盘、基础汇总和复盘 Prompt 生成。Stage 2 Debug 入口仍保留为折叠区块，后续阶段会移除。
